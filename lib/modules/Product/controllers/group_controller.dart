...
class GroupController {
  ...
  Future<DatabaseReference?> getGroupRef() async {
    ...
    await targetedGroupQuery.get().then(
      (groupsSnapshot) {
        for (var group in groupsSnapshot.children) {
          ref = group.ref;
        }
      },
    );
    ...
    await query?.get().then(
      (productsSnapshot) async {
        for (DataSnapshot product in productsSnapshot.children) {
          ...
          Map<String, dynamic> priceListMap = productMap["priceList"];
          ...
          await removeWebImage(productMap["productInfo"]?["url"]);
          await removeWebImage(productMap["offer"]?["url"]);
          for (var locationImage in priceListMap.values) {
            await removeWebImage(locationImage["image"]?["url"]);
          }
        }
      },
    );

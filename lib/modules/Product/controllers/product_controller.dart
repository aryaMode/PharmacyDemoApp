...
class ProductController {
      ...
      (groupsSnapshot) async {
        for (DataSnapshot groupSnapshot in groupsSnapshot.children) {
          ...
          await groupQuery.get().then(
            (productsSnapshot) {
              for (var product in productsSnapshot.children) {
                ref = product.ref;
              }
            },
          );
  ...
  Future<void> setPriceListImage(
    ...
    String? url = locationSnapshot?.child("image/url").value as String?;
    await removeWebImage(url);
    await locationSnapshot?.ref.update({"image": priceListImage.toMap()});
  }
  ...
  Future<void> setOther(PlatformFile otherFile) async {
    Img productInfo = await uploadWebImage(otherFile);
    DatabaseReference? ref = await futureRef;
    String? url =
        (await ref?.child("productInfo").child("url").get())?.value as String?;
    await removeWebImage(url);
    await ref?.update({"productInfo": productInfo.toMap()});
  }

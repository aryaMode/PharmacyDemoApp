...
import 'models/folder.dart' as folder_model;
...
class _TabScreenState extends ConsumerState<TabScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          TabTitles.tabTitles[ref.read(tabTitleIndexProvider)],
        ...
        actions: [
          editable
              //TODO: Fix DropDown and TabChild while Dragging.
              ? Container(
...
class _TabScreenBuilderState extends ConsumerState<TabScreenBuilder> {
  ...
  final dustbinIconProvider =
      StateProvider<IconData>((ref) => Icons.delete_outline_rounded);
  ...
  void _initialization(List tabsData, BuildContext context, bool editable) {
    ...
    bottomDustbin = DragTarget(
      builder: (BuildContext context, List<Object?> candidateData,
          ...
          RoundedIcon(
        iconData: ref.watch(dustbinIconProvider),
        backgroundColor: kSecondaryColor,
        iconColor: kPrimaryColor,
        size: 55,
      ),
      onWillAccept: (data) {
        ref.read(dustbinIconProvider.state).state = Icons.delete_forever;
        return true;
      },
      onLeave: (data) {
        ref.read(dustbinIconProvider.state).state =
            Icons.delete_outline_rounded;
      },
      onAccept: (data) async {
        Map mapData = data as Map;
        if (mapData["type"] == Tab) {
          await TabsController(reader: ref.read).delete(
            index: mapData["index"],
          );

          ref
              .read(tabIndexProvider.state)
              .update((state) => state != 0 ? state - 1 : state);
        } else if (mapData["type"] == folder_model.Folder) {
          await LocationFoldersController(
                  tabIndex: ref.read(tabIndexProvider), reader: ref.read)
              .deleteFolder(
            mapData["index"],
          );
        }
            ...
            Column(
              children: [
                CustomTabs(
                  ...
                  filledOutlineButtonList: tabs.isNotEmpty ? tabs : <Widget>[],
                  footer: editable
                      ? [
                          buildAddButton(
                            onAddPress: () async {
...
class _FoldersViewBuilderState extends ConsumerState<FoldersViewBuilder> {
  late List<Widget>? folders;
  _initialization(List<folder_model.Folder>? foldersData, bool editable) {
    folders = foldersData
        ?.map((folder_model.Folder folder) => Draggable(
              childWhenDragging: const SizedBox.shrink(),
              maxSimultaneousDrags: editable ? 1 : 0,
              data: {
                "type": folder_model.Folder,
                "index": folder.index,
              },
              feedback: Opacity(
                opacity: 0.5,
                ...
                onBackgroundIconPressed: () async {
                  ...
                  if (result != null) {
                    await FolderController(
                            folderIndex: foldersData.indexOf(folder),
                            tabIndex: ref.read(tabIndexProvider),
                            reader: ref.read)
                        .setBackgroundImage(result.files.first);
                  }
                },
                ...
                onPress: () {
                  showDialog(
                    ...
                    builder: (context) => Consumer(
                      builder: (context, reference, _) => DialogBox(
                        child: Container(
                          ...
                          child: ScrollableImages(
                            onAdd: (index) async {
                              await FolderController(
                                      folderIndex: foldersData.indexOf(folder),
                                      tabIndex:
                                          reference.read(tabIndexProvider),
                                      reader: reference.read)
                                  .addImage(index + 1);
                            },
                            ...
                            //TODO: ! operator used when null (Reorderes folders when in a FolderDialog)
                            images: reference
                                .watch(folderProvider(
                                    foldersData.indexOf(folder)))!
                                .images
                                .values
                                .toList(),
                          ),

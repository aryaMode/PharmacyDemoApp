...
class _ProductScreenState extends ConsumerState<ProductScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Group>? groups = ref.watch(groupsProvider);
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: ProviderScope(
          overrides: [productsProvider.overrideWithValue(kjbibiub)],
...
class _ProductScreenBuilderState extends ConsumerState<ProductScreenBuilder> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance?.addPostFrameCallback((_) {
      ref.read(productsProvider.state).state = List<Product>.from(
        widget.groups.expand(
  ...
  _initialization() {
    final selectedProductsRead = ref.read(productsProvider.state);
    allButton = FillOutlineButton(
      ...
      press: () {
        selectedProductsRead.state = List<Product>.from(
            widget.groups.expand((group) => group.products ?? <Product>[]));
      },
    );
    groupTabs = [
      for (int i = 0; i < widget.groups.length; i++)
        FillOutlineButton(
          press: () {
            selectedProductsRead.state = widget.groups[i].products;
          },
  ...
  Widget build(BuildContext context) {
    final editable = ref.watch(editableProvider);
    selectedProductsState = ref.watch(productsProvider.state);
    productSliderTiles = [
          ...
          children: [
                      ...
                      buildAddButton(
                        onAddPress: () {
                          ...
                          widget.groups.add(Group(
                              ...
                              products: [Product.emptyProduct(0)]));
                          groupTabs.add(
                            FillOutlineButton(
                              press: () {},
            ...
            Flexible(
              ...
              child: PageView.builder(
                  ...
                  itemCount: selectedProductsState.state.length,
                  onPageChanged: (val) {
            ...
            ? Positioned(
                ...
                child: Padding(
                  ...
                  child: AddRemoveIconRow(
                    ...
                    listLength: selectedProductsState.state.length,
                    onAdd: () {},
                    onDelete: () {},
                  ),
                ),

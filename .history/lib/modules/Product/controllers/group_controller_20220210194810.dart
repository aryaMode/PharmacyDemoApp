    ...
    await query?.get().then(
      (productsSnapshot) async {
        for (DataSnapshot product in productsSnapshot.children) {
          ...
          SplayTreeMap<String, LocationImage> splayTreeMap =
              productMap["priceList"];
          ...
          for (LocationImage locationImage in splayTreeMap.values) {

            await removeWebImage(locationImage.image?.url);
          }
          splayTreeMap.values.forEach((element) async {
          });
        }
      },
    );

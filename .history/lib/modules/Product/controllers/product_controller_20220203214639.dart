...
class ProductController {
  ...
  Future<void> setLogo(File logoFile) async {
    ...
    DatabaseReference? ref = await futureRef;
    String url = (await ref?.child("logo").child("url").get())?.value as String;
    if (product.logo!.url != null) {await removeWebImage(product.logo!.url!,);
                                                  }
    await ref?.update({"logo": newImg.toMap()});
  }

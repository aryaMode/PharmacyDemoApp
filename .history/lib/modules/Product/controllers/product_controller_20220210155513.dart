...
class ProductController {
  ...
  Future<void> setPriceListImage(PlatformFile priceListFile, Reader reader) async {
    Img priceList = await uploadWebImage(priceListFile);
    DatabaseReference? ref = await futureRef;
    DataSnapshot? dataSnapshot =
        await ref?.child("priceList").orderByChild("location").equalTo(Locations.locations[reader(locationIndexProvider)]).get();
        
String? url = dataSnapshot?.children.first.child("url").value as String?;
      await removeWebImage(url);

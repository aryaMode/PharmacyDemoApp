                      ...
                      onDelete: () async {
                        ...
                        if(selectedProducts.isEmpty){
                          if(pageIndexState.state !=0){
                            pageIndexState.update((state) => null)
                          }
                        }
                        ref.read(selectedGroupsIndicesProvider.state).update(
                            (state) => (selectedProducts.isEmpty && state != 0)
                                ? state - 1
                                : state);
                        ref.read(selectedGroupsIndicesProvider.state).update((state) => );
                        if (selectedProducts.isEmpty)
                          pageController.animateToPage(
                            pageIndexState.state,
                            duration: const Duration(milliseconds: 400),

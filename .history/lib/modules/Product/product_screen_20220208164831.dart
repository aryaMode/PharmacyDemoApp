      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          ref.watch(editableProvider)
              ? Visibility(
                  visible: ref.watch(selectedGroupIndicesProvider) != -1,
              ...
              builder: (context, reference, _) => DialogBox(
                child: ReorderableLayout(
                  ...
                  onReorder: (int oldIndex, int newIndex) {
                    GroupController(
                            index:
                                reference.read(selectedGroupIndicesProvider))
                  ...
                  children: reference
                      ...
                      .map((Product product) => Folder(
                            ...
                            text: product.name,
                            backgroundImage:
                                ImageWidget(image: product.images.values.first),
                          ))
                      .toList(),
                ),
              ),
  ...
  _initialization(List<Group> groups) {
    allButton = FillOutlineButton(
      ...
      press: () {
        ...
        ref.read(selectedGroupIndicesProvider.state).state = -1;
      },
    );
    groupTabs = groups.map(
      (group) {
        return FillOutlineButton(
          press: () {
            ref.refresh(pageIndexProvider);
            ref.read(selectedGroupIndicesProvider.state).state =
                    ...
                    child: AddRemoveIconRow(
                      ...
                      onAdd: () async {
                        await GroupController(
                                index: ref.read(selectedGroupIndicesProvider))
                      ...
                      onDelete: () async {
                        ...
                        ref.read(selectedGroupIndicesProvider.state).update(
                            (state) =>

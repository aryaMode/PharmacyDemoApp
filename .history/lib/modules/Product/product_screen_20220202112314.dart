...
class _ProductScreenBuilderState extends ConsumerState<ProductScreenBuilder> {
  ...
  @override
  Widget build(BuildContext context) {
    final asyncSelectedProducts = ref.watch(productsProvider);
    final editable = ref.watch(editableProvider);

    asyncSelectedProducts.when(data: data, error: error, loading: loading)
    productSliderTiles = [
    ...
    asyncGroups.when(
          data: (groups) {
            ...
            print(ref.read(groupsProvider).map((e) => e.toMap()));
            return const ProductScreenBuilder();
          },

...
class Group {
  ...
  factory Group.emptyGroup(int index) {
    return Group(
      index: index,
      name: 'Untitled',
      products: SplayTreeMap<String, Product>.from(
        {
          Uuid().v4(): Product.emptyProduct(index: 0),
        },
      ),
    );
  }
  ...
  factory Group.fromMap(Map<String, dynamic> map) {
    return Group(
      name: map['name'] ?? '',
      products: SplayTreeMap<String, dynamic>.from(map["products"],
          (String a, String b) {
        return map["products"][a]["index"] - map["products"][b]["index"];
      }).map((key, value) => MapEntry(key, Product.fromMap(value))),
      // products: SplayTreeMap<String, Product>.fromMap(map['products']),

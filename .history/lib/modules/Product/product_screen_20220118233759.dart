...
class ProductScreenBuilder extends ConsumerStatefulWidget {
  ...
  const ProductScreenBuilder({required this.groups, Key? key }) : super(key: key);
...
class _ProductScreenBuilderState extends ConsumerState<ProductScreenBuilder> {
  ...
  late dynamic selectedProducts;
  _initialization() {
    selectedProducts = ref.read(productsProvider.state);
    allButton = FillOutlineButton(
      text: "All",
      backgroundColor: kSecondaryColor,
      textColor: Colors.white,
      press: () {
        ref.read(productsProvider.state).state = (widget.groups.expand((group) => group.products)).toList();
      },
    );
    productTabs = [
      for (int i = 0; i < widget.groups.length; i++)
        FillOutlineButton(
          press: () {
            ref.read(productsProvider.state).state = widget.groups[i].products;
          },
  ...
  Widget build(BuildContext context) {
    ...
    return ProviderScope(
      overrides: [productsProvider.overrideWithValue(StateController((widget.groups.expand((group) => group.products)).toList()))],
      child: Stack(
        children: [
          Column(
            children: [

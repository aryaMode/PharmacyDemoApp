      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          ref.watch(editableProvider)
              ? Visibility(
                  visible: ref.watch(selectedGroupIndexProvider) != -1,
                  child: const ReorderIcon(),
                )
              : SizedBox.shrink(),
          EditIcon(
...
class ReorderIcon extends ConsumerWidget {
  const ReorderIcon({Key? key}) : super(key: key);
              ...
              builder: (context, reference, _) => DialogBox(
                child: ReorderableLayout(
                  ...
                  onReorder: (int oldIndex, int newIndex) {
                    GroupController(
                            index: reference.read(selectedGroupIndexProvider))
                  ...
                  children: reference
                      ...
                      .map((Product product) => Folder(
                            editable: false,
                            text: product.name,
                            backgroundImage: product.images.values.first,
                          ))
                      .toList(),
                ),
              ),
  ...
  _initialization(List<Group> groups) {
    allButton = FillOutlineButton(
      ...
      press: () {
        ...
        ref.read(selectedGroupIndexProvider.state).state = -1;
      },
    );
    groupTabs = groups.map(
      (group) {
        return FillOutlineButton(
          press: () {
            ref.refresh(pageIndexProvider);
            ref.read(selectedGroupIndexProvider.state).state =
                groups.indexOf(group);
            ref.read(pageIndexProvider.state).state = 0;
          },
          text: group.name,
...
class _ProductPageViewState extends ConsumerState<ProductPageView> {
  final PageController pageController = ref.watch(provider);
                    ...
                    child: AddRemoveIconRow(
                      ...
                      onAdd: () async {
                        await GroupController(
                                index: ref.read(selectedGroupIndexProvider))
                      ...
                      onDelete: () async {
                        ...
                        ref.read(selectedGroupIndexProvider.state).update(
                            (state) =>

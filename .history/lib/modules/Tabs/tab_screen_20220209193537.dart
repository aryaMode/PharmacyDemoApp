...
class _TabScreenState extends ConsumerState<TabScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: asyncTabs.when(
          data: (tabsData) {
            tabsData.sort((a, b) => (a.index.compareTo(b.index)));

            return TabScreenBuilder(tabsData: tabsData);
          },
...
class TabScreenBuilder extends ConsumerStatefulWidget {
  ...
  const TabScreenBuilder({required this.tabsData, Key? key}) : super(key: key);
...
class _TabScreenBuilderState extends ConsumerState<TabScreenBuilder> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance?.addPostFrameCallback(
      (_) {
        ref.read(tabsProvider.state).state = widget.tabsData;
      },
    );
  }
  ...
  Widget build(BuildContext context) {
    final editable = ref.watch(editableProvider);
    final tabsData = ref.watch(tabsProvider);
        ...
        Column(
          children: [
            CustomTabs(
              ...
              filledOutlineButtonList: tabs!=null && tabs.length!= ? <Widget>[],
              footer: editable
                  ? [
                      buildAddButton(
                        onAddPress: () async {
                          int newTabs = tabsData
                              .where((tab) => tab.name.startsWith("New tab"))
                              .length;

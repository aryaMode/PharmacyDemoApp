...
import 'models/folder.dart' as folderModel;
      ...
      appBar: AppBar(
        ...
        actions: [
          editable
              ...
              ? Container(
                  padding: EdgeInsets.symmetric(horizontal: kDefaultPadding vertical: kdefa),
  ...
  void _initialization(List tabsData, BuildContext context, bool editable) {
    ...
    bottomDustbin = DragTarget(
      ...
      onAccept: (data) async {
        ...
        } else if (mapData["type"] == folderModel.Folder) {
            ...
            Column(
              children: [
                CustomTabs(
                  ...
                  filledOutlineButtonList: tabs.length != 0 ? tabs : <Widget>[],
                  footer: editable
                      ? [
                          buildAddButton(
                            onAddPress: () async {
...
class _FoldersViewBuilderState extends ConsumerState<FoldersViewBuilder> {
  late List<Widget>? folders;
  _initialization(List<folderModel.Folder>? foldersData, bool editable) {
    folders = foldersData
        ?.map((folderModel.Folder folder) => Draggable(
              childWhenDragging: SizedBox.shrink(),
              maxSimultaneousDrags: editable ? 1 : 0,
              data: {
                      ...
                      builder: (context, reference, _) => DialogBox(
                        child: Container(
                          ...
                          child: ScrollableImages(
                            ...
                            images: reference
                                .watch(
                                    folderProvider(foldersData.indexOf(folder)))
                                ?.images
                                ?.values
                                .toList(),
                          ),

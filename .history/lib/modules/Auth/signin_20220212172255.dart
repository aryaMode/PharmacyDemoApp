...
class _SignInState extends ConsumerState<SignIn> {
                  ...
                  child: AuthPageTemplate(
                    child: Column(
                      children: [
                        ...
                        CustomFormField(
                          onSubmitted: (submittedText) async {
                            await signInButtonAction();
                          },
                          hintText: "Password",
  ...
  Future<void> signInButtonAction() async {
    try {
      ...
      await AuthServices.authSignin(email: email, password: password)
          .then((_) async {
        ...
        ref.read(userProvider.state).state = AuthUser.fromMap(currentUser!);
        if (!kIsWeb) {
          SharedPreferences prefs = await SharedPreferences.getInstance();
          await prefs.setStringList("locations", Locations.locations);
          await prefs.setStringList("tabTitles", TabTitles.tabTitles);
          await prefs.setBool("admin", currentUser["admin"]);
          await prefs.setString("location", currentUser["location"]);
        ...
        if (kIsWeb && !ref.read(userProvider)!.admin) {
          ...
          AutoRouter.of(context).replace(SplashScreenRoute());
        }
      });
